...
## 1. Product contract
- [x] Every event produces: event record, structured surprise, guidance-baseline comparison, expected factor move, actual move, residual, hedge ratio, trade/no-trade decision, paper outcome (orders when traded, explicit no-trade otherwise), and post-event attribution (`data/results.json`, dashboard)
...
- [x] Insufficient data or no valid hedge → NO_TRADE (8 events rejected for market data in the current dataset)
...
- [~] Analyst consensus: not available in the SEC filing; the product explicitly uses a cited company-guidance baseline (prior-quarter outlook midpoint), not a fabricated consensus estimate
...
- [~] Funding: Bitget serves history only from about June 2026; earlier trades are flagged `unavailable_for_period` and excluded from primary aggregates. An observed-settlements-only view is retained separately.
...
- [x] 8 Paper executor: both legs, prices, quantities, fees, slippage, funding, balance; unavailable funding is labeled and not silently treated as observed
...
- [x] Results regenerate from code (`python -m residual replay --offline`) without SEC, Bitget, or LLM network calls; committed interpretation caches are required
...
- [x] Dashboard deployed: https://residual-teal.vercel.app (static snapshot; local watcher updates are not automatically deployed)
- [x] Private execution boundary isolated in `residual/execution.py`: it only builds a signed Demo read request with `paptrading: 1`; it sends no request and exposes no order method
- [ ] Fresh Bitget Demo credentials and read-only account verification remain operator-dependent: `BITGET_API_KEY`, `BITGET_SECRET_KEY`, `BITGET_PASSPHRASE`
